...
class VentaRepository:
    ...
    def crear_venta(self, venta: Venta) -> bool:
        ...
        try:
            ...
            # 1. Insertar venta
            query_venta = (
                "INSERT INTO ventas (total, estado, cliente_id) VALUES (?, ?, ?)"
            )
            ...
            for item in venta.items:
                cursor.execute(
                    query_item,
                    (venta_id, item.producto_id, item.cantidad, item.subtotal),
                )
    ...
    def listar_ventas(self) -> list[Venta]:
        ...
        try:
            ...
            for row in rows:
                ventas.append(
                    Venta(id=row[0], total=row[1], estado=row[2], cliente_id=row[3])
                )
        except Exception as e:
    ...
    def obtener_venta_por_id(self, id_venta: int) -> Optional[Venta]:
        ...
        try:
            ...
            # Obtener cabecera
            cursor.execute(
                "SELECT id, total, estado, cliente_id FROM ventas WHERE id = ?",
                (id_venta,),
            )
            row = cursor.fetchone()
            if row:
                ...
                # Obtener items
                cursor.execute(
                    "SELECT id, venta_id, producto_id, cantidad, subtotal FROM lineas_venta WHERE venta_id = ?",
                    (id_venta,),
                )
